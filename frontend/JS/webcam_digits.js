document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load TensorFlow Digits Model
    try {
        if (statusText) statusText.textContent = "Loading Digits Model...";
        // Load the model locally using TFJS with cache buster
        model = await tf.loadLayersModel('digits_web_model/model.json?t=' + new Date().getTime());
        
        // Load class indices
        const response = await fetch('digits_web_model/digits_class_indices.json');
        const data = await response.json();
        classIndices = data.index_to_class || data; 
        isModelLoaded = true;
        
        if (statusText) statusText.textContent = "Digits Model Loaded. Starting camera...";
    } catch (e) {
        console.error("Error loading Digits model:", e);
        if (statusText) {
            statusText.textContent = `Error loading Digits model: ${e.message}`;
    ...
    hands.onResults((results) => {
        ...
        // Exactly matches the Python training pipeline in DigitsModel.ipynb
        const inputData = [];
        let maxDist = 0;
        ...
        tf.tidy(() => {
            ...
            const predictedDigit = classIndices[maxIndex.toString()] || classIndices[maxIndex];

            if (predictedDigit !== undefined && predictedDigit !== null) {
                // Buffer to stabilize predictions
                predictionBuffer.push({ letter: String(predictedDigit), score: maxScore });
                ...
                const counts = {};
                let maxCount = 0;
                let stableDigit = String(predictedDigit);
                ...
                predictionBuffer.forEach(p => {
                    ...
                    if (counts[p.letter] > maxCount) {
                        ...
                        stableDigit = p.letter;
                    }
                });

                const stablePredictions = predictionBuffer.filter(p => p.letter === stableDigit);
                ...
                if (window.handleDetection) {
                    window.handleDetection(stableDigit, avgScore);
                }
            }
        });
    });
